...
def is_match(s1, s2, unk_char):
    s1 = s1.replace('##', unk_char)
    s2 = s2.replace('##', unk_char)
    N1 = len(s1)
    N2 = len(s2)
    if N1 == 0 and N2 == 0:
        return True
    if N1 > 0 and N2 == 0:
        return False
    if N1 == 0 and N2 > 0:
        return all([s == unk_char for s in s2])

    DP = [[False for _ in range(N2)] for _ in range(N1)]
    DP[0][0] = True if s1[0] == s2[0] or s2[0] == unk_char else False
    for i in range(1, N2):
        DP[0][i] = DP[0][i-1] if s2[i] == unk_char or (s2[i-1] == unk_char and s1[0]==s2[i]) else False
    for j in range(1, N1):
        for i in range(1, N2):
            if s1[j] == s2[i]:
                DP[j][i] = DP[j-1][i-1]
...
def compare(pred, label, with_punctuation=True, use_dp=False):
    ...
    cer = levenshteinDistance(depunc(replaceFullToHalf(pred)), depunc(replaceFullToHalf(label)))
    ...
    if use_dp:
        unk_char = '[unk]'
        return is_match(''.join(pred), ''.join(label), unk_char), cer
